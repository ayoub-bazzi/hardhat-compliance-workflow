...
export async function reviewCertificate(
  ...
  const { data: cert } = await supabase
    ...
    .select('id, status, subcontractor_id, organization_id, amount')
    .eq('id', certId)
    .single()
  ...
  await supabase.rpc('fn_log_audit_event', {
    ...
    p_description:      newStatus === 'escrowed'
      ? `Certificate escrowed by ${actor} — risk score ${riskScore} > 30.`
      : `Certificate approved by ${actor} (risk ${riskScore}).`,
    p_actor: actor,
    p_metadata: { cert_id: certId, new_status: newStatus, risk_score: riskScore, amount: cert.amount },
  })
...
export async function releaseCertificate(
  ...
  await supabase.rpc('fn_log_audit_event', {
    ...
    p_organization_id:  cert.organization_id,
    p_event_type:       'Payment Update',
    p_description:      `Certificate ($${cert.amount}) released by ${actor}. Reason: "${overrideReason.trim()}"`,
    p_actor: actor,
    p_metadata: { cert_id: certId, override_reason: overrideReason.trim(), amount: cert.amount },
  })
...
export async function addPaymentCertificate(formData: FormData): Promise<ActionResult> {
  ...
  const from   = formData.get('period_start') as string
  const to     = formData.get('period_end') as string

  if (!subId || isNaN(amount) || !from || !to) {
